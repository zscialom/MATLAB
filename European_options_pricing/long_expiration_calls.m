...
calibrate_option_price_call = zeros(10,10);                                % tab containing the option prices for every strike and every u
T = 70;                                                                    % maturity
...
err = zeros(1,length(calibrate_option_price_call(1,:))); % vector containg all the errors
for i=1:length(calibrate_option_price_call(1,:))
    for j=1:length(calibrate_option_price_call)
        err(1,i) = err(1,i) + sqrt((market_option_price_call(15+j)-calibrate_option_price_call(j,i)).^2); %RMSE
    end
end
...
%% Strike vs Prices
figure;
plot(tab_option_strike_chosen_call,calibrate_option_price_call(:,7),'r')
hold on
